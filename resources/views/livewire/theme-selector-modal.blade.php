<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-12 px-4" wire:key="theme-selector">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-12">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('filament.admin.pages.settings') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition" style="background-color: #111827; color: white;" onmouseover="this.style.backgroundColor='#0b1220'" onmouseout="this.style.backgroundColor='#111827'">
            ...
            @foreach ($themes as $theme)
                ...
                >
                        ...
                        @if (!empty($theme['colors']))
                            <div class="flex items-center gap-2">
                                ...
                                <div class="flex gap-1">
                                    @foreach (array_slice($theme['colors'], 0, 4) as $colorKey => $colorValue)
                    ...
                    <div class="p-6 flex flex-col gap-3">
                        ...
                        <button
                            type="button"
                            wire:click="openPreview('{{ $theme['id'] }}')"
                            class="w-full py-2 px-4 rounded-lg font-medium transition duration-200"
    ...
    <!-- Modal Anteprima -->
    @if ($showPreviewModal !== null && !empty($showPreviewModal))
        @php
            $previewData = collect($themes)->firstWhere('id', $showPreviewModal);
        @endphp
        
        @if ($previewData)
            <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 transition-opacity" wire:key="modal-{{ $showPreviewModal }}">
                <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                    ...
                    <div class="bg-gray-900 text-white px-8 py-4 flex items-center justify-between">
                        <h2 class="text-2xl font-bold">Anteprima: {{ $previewData['name'] }}</h2>
                        <button
                            type="button"
                    ...
                    <div class="flex-1 overflow-y-auto bg-white dark:bg-gray-900">
                        <iframe
                            x-init="$el.srcdoc = `<!doctype html><html lang='it'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'>` +
                                ...
                                `</head><body class='bg-white'>` + @js($previewData['html_content']) + `</body></html>`"
