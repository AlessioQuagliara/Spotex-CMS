                    ...
                    <div class="border border-gray-200 rounded-none p-8 bg-white">
                        ...
                        <div class="space-y-4">
                            ...
                            @forelse($shippingRules as $index => $rule)
                                <div class="relative">
                                    <input type="radio" name="shipping_method" id="shipping_{{ $rule->id }}" 
                                           value="{{ $rule->type }}" {{ $index === 0 ? 'checked' : '' }} 
                                           class="absolute opacity-0 h-0 w-0">
                                    <label for="shipping_{{ $rule->id }}" 
                                           class="block border-2 {{ $index === 0 ? 'border-black' : 'border-gray-300' }} p-6 rounded-none cursor-pointer hover:border-gray-500 transition-colors duration-200">
                                        <div class="flex justify-between items-center">
                                            <div class="flex items-center">
                                                <div class="w-6 h-6 border-2 {{ $index === 0 ? 'border-black' : 'border-gray-400' }} flex items-center justify-center mr-4">
                                                    @if($index === 0)
                                                        <div class="w-3 h-3 bg-black"></div>
                                                    @endif
                                                </div>
                                                <div>
                                                    <div class="font-bold text-lg text-black">{{ $rule->name }}</div>
                                                    @if($rule->description)
                                                        <div class="text-sm text-gray-600 mt-1">{{ $rule->description }}</div>
                                                    @endif
                                                    @if($rule->estimated_days)
                                                        <div class="text-xs text-gray-500 mt-1">Consegna stimata: {{ $rule->estimated_days }} giorni lavorativi</div>
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="text-lg font-bold text-black">€{{ number_format($rule->base_cost, 2, ',', '.') }}</div>
                                        </div>
                                    </label>
                                </div>
                            @empty
...
<script type="application/json" id="shipping-rules">@json($shippingRules ?? [])</script>
<script type="application/json" id="user-data">@json($user ?? [])</script>
    ...
    if (cart.length === 0) {
        summaryContainer.innerHTML = `
            <div class="text-center py-8">
                ...
                <a href="{{ route('products') }}" class="text-black underline text-sm mt-2 inline-block">Continua lo shopping</a>
            </div>
        `;
        currentTotal = 0;
        updateTotals(0);
...
async function submitStripePayment() {
    ...
    try {
        // Get client secret
        const response = await fetch('{{ route("payment.stripe.checkout") }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
