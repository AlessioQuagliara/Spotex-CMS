...
<body class="bg-white">
    <!-- Navigation -->
    <nav id="main-nav" class="fixed top-0 left-0 right-0 shadow-lg z-50" style="background-color: {{ $navBg }}; color: {{ $navText }};">
                        ...
                        @if($customerUser)
                            <div class="relative group">
                                ...
                                <div class="hidden group-hover:block absolute right-0 w-48 bg-white text-gray-800 rounded shadow-lg py-2 z-20">
                                    ...
                                    @endif
                                    <a href="{{ route('customer.profile') }}" class="block px-4 py-2 hover:bg-gray-100">Il mio profilo</a>
                                    <a href="{{ route('customer.orders') }}" class="block px-4 py-2 hover:bg-gray-100">I miei ordini</a>
            ...
            <div id="mobileMenu" class="hidden md:hidden mt-4 space-y-2 pb-4">
                ...
                @if($customerUser)
                    <div class="border-t pt-2 mt-2">
                        <div class="px-4 py-2 text-sm font-medium">{{ $customerUser->name }}</div>
                        @if(!$customerUser->hasVerifiedEmail())
                            <div class="px-4 py-1 text-xs bg-yellow-50 text-yellow-800">⚠️ Email non verificata</div>
                        @endif
                        <a href="{{ route('customer.profile') }}" class="block px-4 py-2 rounded hover:opacity-80 transition">Il mio profilo</a>
                        <a href="{{ route('customer.orders') }}" class="block px-4 py-2 rounded hover:opacity-80 transition">I miei ordini</a>
                        @if($showEditorBanner)
        ...
        <div id="editor-banner" class="bg-blue-600 text-white shadow-lg fixed left-0 right-0 z-40" style="top: var(--layout-nav-height);">
            <div class="container mx-auto px-4 py-3">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ $editorTargetUrl }}" class="flex items-center gap-2 bg-white text-blue-600 px-4 py-2 rounded hover:bg-blue-50 transition font-medium text-sm">
