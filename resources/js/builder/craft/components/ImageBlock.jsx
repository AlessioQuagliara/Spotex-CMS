...
export function ImageBlock({ src, alt, radius }) {
    const {
    ...
    return (
        <div ref={(ref) => connect(drag(ref))} className={`overflow-hidden ${selected ? 'ring-2 ring-blue-500' : 'ring-1 ring-slate-200'}`} style={{ borderRadius: `${radius}px` }}>
...
function ImageSettings() {
    const { actions: { setProp }, src, alt, radius } = useNode((node) => ({
